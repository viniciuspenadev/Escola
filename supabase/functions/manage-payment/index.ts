...
serve(async (req) => {
    ...
    try {
        ...
        // 1. Validar e Buscar Gateway Config
        ...
        if (settingsError || !settingsData?.value) throw new Error('Gateway não configurado.')

        const config = typeof settingsData.value === 'string' ? JSON.parse(settingsData.value) : settingsData.value
        if (config.provider !== 'asaas' || !config.api_key) throw new Error('Asaas não configurado.')
