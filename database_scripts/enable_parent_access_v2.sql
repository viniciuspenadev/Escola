-- RLS Policies for Parent Portal Access (Corrected Tables)
...
-- 4. Grade Books (So they can see what the grade is for)
-- Allow access if the parent has a child in the class associated with the grade book
...
USING (
    EXISTS (
        SELECT 1 FROM public.enrollments e
        JOIN public.student_guardians sg ON sg.student_id = e.student_id
        WHERE e.class_id = grade_books.class_id
        AND sg.guardian_id = auth.uid()
    )
);
