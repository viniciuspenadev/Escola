...
export const DailyTimelineComponent: FC<DailyTimelineProps> = ({ classId, enrollmentId }) => {
    ...
    return (
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 animate-fade-in my-1">
            ...
            <div className="relative">
                <div className="flex overflow-x-auto gap-0.5 pb-0 scrollbar-hide snap-x px-0 justify-between">
                    {(timeline.items || []).map((item, index) => {
                        const isLast = index === (timeline.items?.length || 0) - 1;
                        const nextItem = !isLast ? timeline.items![index + 1] : null;
                        ...
                        return (
                            <div key={item.id} className={`relative flex-none md:flex-1 w-[52px] md:w-auto md:min-w-[52px] flex flex-col items-center group snap-start transition-opacity duration-300 ${isFuture ? 'opacity-50 grayscale' : 'opacity-100'}`}>
