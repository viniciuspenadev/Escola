...
export const ClassAttendance: FC<ClassAttendanceProps> = ({ classId, date }) => {
    ...
    const handleSave = async () => {
        if (!isComplete) {
            addToast('error', 'Por favor, preencha o status de todos os alunos.');
            return;
        }
    ...
    return (
        <div className="space-y-6 animate-fade-in-up">
            {/* Control Bar */}
            <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm space-y-4 sticky top-4 z-20">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex gap-3 w-full md:w-auto">
                        ...
                        <Button
                            ...
                            className={`flex-1 md:flex-none min-w-[160px] text-white transition-all shadow-lg
                                ${isComplete ? 'bg-brand-600 hover:bg-brand-700 hover:scale-105' : 'bg-gray-300 cursor-not-allowed'}
                            `}
                        >
