...
export const FinancialStudentHub: FC = () => {
    ...
    const fetchStudentDetails = async (studentId: string) => {
        ...
        try {
            const { data: enrollments } = await supabase
                .from('enrollments')
                .select('id, candidate_name, details')
    ...
    // -- Render --
    return (
        <div className="h-[calc(100vh-100px)] flex flex-col md:flex-row gap-6 animate-fade-in relative">

            {/* Left Panel: Student List */}
            <Card className={`flex-1 flex flex-col h-full overflow-hidden ${selectedStudentId ? 'hidden md:flex' : 'flex'}`}>
                {/* Visual Management Header (KPIs) */}
            ...
            {/* Right Panel: Details */}
            <div className={`flex-[2] flex flex-col h-full overflow-hidden ${!selectedStudentId ? 'hidden md:flex' : 'flex'}`}>
                {selectedStudentId ? (
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <p className="font-semibold text-gray-900">{inst.installment_number}ª Mensalidade</p>
