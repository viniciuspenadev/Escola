        ...
        <div className="space-y-8 animate-fade-in pb-20">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center bg-white p-2 rounded-xl border border-gray-200 shadow-sm">
                <div className="flex-1 w-full relative group">
                    <Search className="absolute left-3 top-3 w-5 h-5 text-gray-400 group-focus-within:text-brand-500 transition-colors" />
                    <input
                        type="text"
                        placeholder="Buscar por nome da turma..."
                        className="w-full pl-10 pr-4 py-2.5 border-transparent rounded-lg text-gray-700 placeholder-gray-400 focus:ring-0 bg-transparent outline-none"
                        value={searchTerm}
                ...
                <select
                    className="w-full md:w-auto px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 focus:ring-2 focus:ring-brand-500 outline-none cursor-pointer hover:bg-gray-100 transition-colors"
                    value={yearFilter}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {loading ? (
                    ...
                    filteredClasses.map((cls) => {
                        ...
                        // Teacher Helper
                        const primaryTeacher = (cls as any).teachers?.find((t: any) => t.is_primary);
                        const otherTeachersCount = (cls as any).teachers?.length - (primaryTeacher ? 1 : 0);

                        return (
                            <div
                                key={cls.id}
                                onClick={() => navigate(`/turmas/${cls.id}`)}
                                className="group bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-xl hover:border-brand-200 transition-all duration-300 cursor-pointer relative overflow-hidden flex flex-col"
                            >
                                ...
                                <div className="p-6 flex-1">
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-xl bg-brand-50 text-brand-600 flex items-center justify-center font-bold text-xl shadow-inner">
                                                {cls.name.substring(0, 2).toUpperCase()}
                                            </div>
                                            <div>
                                                <h3 className="font-bold text-lg text-gray-900 group-hover:text-brand-600 transition-colors leading-tight">{cls.name}</h3>
                                        ...
                                        <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100/50">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-sm font-medium text-gray-900 truncate">
                                                    {primaryTeacher ? primaryTeacher.name : <span className="text-gray-400 italic">Não atribuído</span>}
                                                </p>
                                            </div>
                                ...
                                <div className="bg-gray-50 px-6 py-4 border-t border-gray-100 flex justify-between items-center group-hover:bg-brand-50/30 transition-colors">
                                    <span className="text-sm font-semibold text-gray-600 group-hover:text-brand-700 transition-colors">
                                        Gerenciar Turma
                                    </span>
                                    <div className="w-8 h-8 rounded-full bg-white border border-gray-200 flex items-center justify-center group-hover:border-brand-300 group-hover:text-brand-600 transition-all shadow-sm">
