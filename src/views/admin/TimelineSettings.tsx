...
export const TimelineSettings: FC = () => {
    ...
    const { value: displayMode, updateSetting: setDisplayMode } = useAppSettings('daily_timeline_display_mode', 'card');
        ...
        <div className="h-[calc(100vh-6rem)] flex flex-col animate-fade-in">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mb-6 flex justify-between items-center animate-fade-in">
                <div>
                    ...
                    <p className="text-sm text-gray-500">Escolha como a agenda aparece para os responsáveis.</p>
                </div>
                <div className="flex bg-gray-100 p-1 rounded-lg">
                    <button
                        onClick={() => updateDisplayMode('graph')}
                        className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${displayMode === 'graph' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                        Timeline Gráfica
                    </button>
                    <button
                        onClick={() => updateDisplayMode('card')}
                        className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${displayMode === 'card' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                        Cartão de Destaque
                    </button>
                    <button
                        onClick={() => updateDisplayMode('disabled')}
                        className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${displayMode === 'disabled' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                        Desativado
                    </button>
                </div>
