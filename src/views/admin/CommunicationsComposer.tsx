...
const CommunicationsComposer: React.FC = () => {
    ...
    const [targetType, setTargetType] = useState<'SCHOOL' | 'CLASS' | 'STUDENT'>('SCHOOL');
    ...
    // const [step, setStep] = useState<1 | 2>(1); // Removed unused step state
    ...
    // Search Effect for Students
    useEffect(() => {
        const delayDebounceFn = setTimeout(async () => {
            if (targetType === 'STUDENT' && searchTerm.length > 2) {
                setIsSearching(true);
    ...
    const handleSend = async () => {
        if (!title || !content || !selectedChannelId) return;
        ...
        try {
            ...
            if (interactionType === 'RSVP') {
                ...
                metadata.options = pollOptions.filter(o => o.trim() !== ''); // Remove empty
            }

            const { data: newCommId, error } = await supabase.rpc('send_communication', {
                p_channel_id: selectedChannelId,
                p_sender_profile_id: user?.id,
                p_title: title,
                p_preview_text: content.substring(0, 140),
                p_content: content.replace(/\n/g, '<br/>'), // Basic processing
                p_priority: priority,
                p_allow_reply: true,
                p_attachments: [],
                p_metadata: metadata,
                p_target_type: targetType,
                p_target_ids: targetType === 'CLASS' ? selectedClasses : (targetType === 'STUDENT' ? selectedStudents.map(s => s.id) : [])
            });

            if (error) throw error;

            // FALLBACK PATCH: Ensure sender_profile_id is set
            // The RPC might not be setting it correctly in the current version on DB
            if (user?.id && newCommId) {
                await supabase
                    .from('communications')
                    .update({ sender_profile_id: user.id })
                    .eq('id', newCommId);
            } else if (user?.id) {
                // If RPC didn't return ID, try to find the match by timestamp (less safe but better than nothing)
                await supabase
                    .from('communications')
                    .update({ sender_profile_id: user.id })
                    .eq('title', title)
                    .eq('channel_id', selectedChannelId)
                    .gt('created_at', new Date(Date.now() - 10000).toISOString()); // Created in last 10s
            }

            setSuccess(true);
            setTimeout(() => {
                setSuccess(false); // Keep this to reset success state
                setTitle('');
                setContent('');
                ...
                <div className="space-y-8 flex-1">
                                ...
                                return (
                                    <button
                                        ...
                                        className={`
                                            ...
                                            ${isSelected
                                                ? `border-${channel.color}-500 bg-${channel.color}-50 text-${channel.color}-700 shadow-md`
                                    ...
                                    >
                                        ...
                                        {isSelected && (
                                            <div className={`absolute top-2 right-2 w-3 h-3 rounded-full bg-${channel.color}-500`} />
                                        )}
                                    </button>
                                );
                            })}
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="p-1.5 bg-gray-100 rounded-xl flex gap-1">
                            {(['SCHOOL', 'CLASS', 'STUDENT'] as const).map((type) => (
                                <button
                                    key={type}
